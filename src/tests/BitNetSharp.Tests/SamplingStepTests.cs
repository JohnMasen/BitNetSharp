...
namespace BitNetSharp.Tests
{
    [TestClass]
    [DoNotParallelize]
    public sealed class SamplingStepTests
    {
        ...
        [TestMethod]
        public void SamplingStep_DefaultTopK_IsTen()
        {
            var step = new BitNetSharp.Layers.SamplingStep();

            Assert.AreEqual(10, step.TopK);
        }

        [TestMethod]
        public void SamplingStep_InvalidTopK_Throws()
        {
            Assert.ThrowsExactly<ArgumentOutOfRangeException>(() => new BitNetSharp.Layers.SamplingStep(0));
        }

        [TestMethod]
        public void SamplingStep_MatchesBaseline_DebugCase()
        {
        ...
        public void SamplingStep_MatchesBaseline(int caseIndex)
        {
            VerifySamplingMatchesBaseline(caseIndex);
        }

        [TestMethod]
        public void SamplingStep_ForwardWithoutInit_Throws()
        {
        ...
        private static void VerifySamplingMatchesBaseline(int caseIndex)
        {
            ...
            var step = new BitNetSharp.Layers.SamplingStep(testCase.LmHead.TopKTokenIds.Length);
            var session = TestModelFactory.CreateSession(model, token: testCase.TokenId);
            testCase.LmHead.Logits.CopyTo(session.Logits.Span);
