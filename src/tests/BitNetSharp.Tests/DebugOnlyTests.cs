...
namespace BitNetSharp.Tests
{
    ...
    public sealed class DebugOnlyTests
    {
        ...
        public void HiChatPrompt_FullRuntimeChain_MatchesDumpUntilFirstMismatch()
        {
            ...
            AssertVectorPrefix("embedding_output", FullDumpBaseline.ReadFloatValues("embedding_output_first_32"), RuntimeTensorBufferExtensions.GetMemory<float>(session.Embedding).Span);
            ...
            for (int layerIndex = 0; layerIndex < model.Layers.Count; layerIndex++)
            {
                ...
                if (layerIndex == 0)
                {
                    AssertVectorPrefix("first_layer_attention_norm_output", FullDumpBaseline.ReadFloatValues("first_layer_attention_norm_output_first_32"), RuntimeTensorBufferExtensions.GetMemory<float>(session.RmsNorm).Span);
                }

                InvokePrivateMethod(runtime, "ExecuteQKVProjection", session, layerIndex, layer);
                if (layerIndex == 0)
                {
                    AssertVectorPrefix("first_layer_qkv_query_output", FullDumpBaseline.ReadFloatValues("first_layer_qcur_before_rope_first_32"), RuntimeTensorBufferExtensions.GetMemory<float>(session.QKVQuery).Span);
                    AssertVectorPrefix("first_layer_qkv_key_output", FullDumpBaseline.ReadFloatValues("first_layer_kcur_before_rope_first_32"), RuntimeTensorBufferExtensions.GetMemory<float>(session.QKVKey).Span);
                    AssertVectorPrefix("first_layer_qkv_value_output", FullDumpBaseline.ReadFloatValues("first_layer_vcur_first_32"), RuntimeTensorBufferExtensions.GetMemory<float>(session.QKVValue).Span);
                    AssertVectorPrefix("qcur_before_rope", FullDumpBaseline.ReadFloatValues("first_layer_qcur_before_rope_first_32"), RuntimeTensorBufferExtensions.GetMemory<float>(session.QKVQuery).Span);
                    AssertVectorPrefix("kcur_before_rope", FullDumpBaseline.ReadFloatValues("first_layer_kcur_before_rope_first_32"), RuntimeTensorBufferExtensions.GetMemory<float>(session.QKVKey).Span);
                    ...
                    float[] ropeQueryValues = ApplyRopeForTest(RuntimeTensorBufferExtensions.GetMemory<float>(session.QKVQuery).Span, positionIndex, headCount, headDimension, ropeDimensionCount, ropeFrequencyBase);
                    float[] ropeKeyValues = ApplyRopeForTest(RuntimeTensorBufferExtensions.GetMemory<float>(session.QKVKey).Span, positionIndex, keyValueHeadCount, headDimension, ropeDimensionCount, ropeFrequencyBase);
                    ...
                    AssertVectorPrefix("first_layer_v_before_kv_cache_write", FullDumpBaseline.ReadFloatValues("first_layer_v_before_kv_cache_write_first_32"), RuntimeTensorBufferExtensions.GetMemory<float>(session.QKVValue).Span, RopeTolerance);

                    WriteKeyCacheForTest(session, layerIndex, ropeKeyValues);
                    WriteValueCacheForTest(session, layerIndex, RuntimeTensorBufferExtensions.GetMemory<float>(session.QKVValue).Span, keyValueHeadCount, headDimension);
                ...
                if (layerIndex == 0)
                {
                    AssertVectorPrefix("first_layer_attention_output", FullDumpBaseline.ReadFloatValues("first_layer_attn_o_out_0_first_32"), RuntimeTensorBufferExtensions.GetMemory<float>(session.AttentionOutput).Span);
                }

                residualNodes[layerIndex].Forward(session);
                if (layerIndex == 0)
                {
                    AssertVectorPrefix("first_layer_feedforward_input", FullDumpBaseline.ReadFloatValues("first_layer_feedforward_input_first_32"), RuntimeTensorBufferExtensions.GetMemory<float>(session.FeedForwardInput).Span);
                }

                feedForwardNormNodes[layerIndex].Forward(session);
                if (layerIndex == 0)
                {
                    AssertVectorPrefix("first_layer_feedforward_norm_output", FullDumpBaseline.ReadFloatValues("first_layer_feedforward_norm_output_first_32"), RuntimeTensorBufferExtensions.GetMemory<float>(session.FeedForwardNorm).Span);
                }
                ...
                if (layerIndex == 0)
                {
                    AssertVectorPrefix("first_layer_feedforward_sub_norm_output", FullDumpBaseline.ReadFloatValues("first_layer_feedforward_sub_norm_output_first_32"), RuntimeTensorBufferExtensions.GetMemory<float>(session.FeedForwardSubNorm).Span);
                    AssertVectorPrefix("first_layer_feedforward_output", FullDumpBaseline.ReadFloatValues("first_layer_feedforward_down_output_first_32"), RuntimeTensorBufferExtensions.GetMemory<float>(session.FeedForwardOutput).Span);
                }

                feedForwardResidualNodes[layerIndex].Forward(session);
                if (layerIndex == 0)
                {
                    AssertVectorPrefix("first_layer_feedforward_output_runtime_semantic", FullDumpBaseline.ReadFloatValues("first_layer_feedforward_output_runtime_semantic_first_32"), RuntimeTensorBufferExtensions.GetMemory<float>(session.Embedding).Span);
                }
            }

            finalNormNode.Forward(session);
            AssertPromptTokenIds(prompt, session);
            AssertVectorPrefix("final_norm_output", FullDumpBaseline.ReadFloatValues("final_norm_output_first_32"), RuntimeTensorBufferExtensions.GetMemory<float>(session.FinalNormOutput).Span);

            lmHeadNode.Forward(session);
            AssertVectorPrefix("lm_head_output_logits", FullDumpBaseline.ReadFloatValues("lm_head_output_logits_first_32"), RuntimeTensorBufferExtensions.GetMemory<float>(session.Logits).Span);
        }
        ...
        private static float[] RunStandaloneFeedForwardNode(Models.BitNetModel model, Models.BitNetLayerDefinition layer, BitNetSession session, InferenceConfig inferenceConfig, string outputVectorName)
        {
            ...
            RuntimeTensorBufferExtensions.GetMemory<float>(session.FeedForwardNorm).Span.CopyTo(RuntimeTensorBufferExtensions.GetMemory<float>(standaloneSession.FeedForwardNorm).Span);
            ...
            AssertVectorPrefix("standalone_feedforward_sub_norm_output", FullDumpBaseline.ReadFloatValues("first_layer_feedforward_sub_norm_output_first_32"), RuntimeTensorBufferExtensions.GetMemory<float>(standaloneSession.FeedForwardSubNorm).Span);
            AssertVectorPrefix(outputVectorName, FullDumpBaseline.ReadFloatValues("first_layer_feedforward_down_output_first_32"), RuntimeTensorBufferExtensions.GetMemory<float>(standaloneSession.FeedForwardOutput).Span);
            return RuntimeTensorBufferExtensions.GetMemory<float>(standaloneSession.FeedForwardOutput).Span.ToArray();
        }
