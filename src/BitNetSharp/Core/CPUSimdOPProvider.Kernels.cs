...
{
    public sealed partial class CPUSimdOPProvider
    {
        private void ExecuteForwardRmsNorm(ReadOnlyMemory<float> input, ReadOnlyMemory<float> normWeights, float epsilon, Memory<float> output)
        {
            ...
            float inverseRootMeanSquare = ComputeRmsNormInverseRootMeanSquare(inputSpan, epsilon);
        ...
        private static int ComputeBitNetMappedDot(ReadOnlySpan<sbyte> quantizedValues, ReadOnlySpan<byte> packedRow)
        {
            ...
            for (int packedIndex = vectorizedPackedLength; packedIndex < packedRow.Length; packedIndex++)
            {
                ...
                mappedDot += quantizedValues[scalarActivationIndex++] * OPProviderCommon.DecodeBitNetWeight(packedWeight, 0);
                mappedDot += quantizedValues[scalarActivationIndex++] * OPProviderCommon.DecodeBitNetWeight(packedWeight, 1);
                mappedDot += quantizedValues[scalarActivationIndex++] * OPProviderCommon.DecodeBitNetWeight(packedWeight, 2);
                mappedDot += quantizedValues[scalarActivationIndex++] * OPProviderCommon.DecodeBitNetWeight(packedWeight, 3);
            }
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private static float ComputeRmsNormInverseRootMeanSquare(ReadOnlySpan<float> input, float epsilon)
        {
