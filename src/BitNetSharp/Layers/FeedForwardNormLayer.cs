...
namespace BitNetSharp.Layers
{
    ...
    /// </summary>
    public sealed class FeedForwardNormLayer
    {
        ...
        public FeedForwardNormLayer(BitNetModel model, BitNetTensorInfo normTensor, bool enableCache = false, InferenceConfig? inferenceConfig = null)
        {
            ...
            {
                throw new InvalidOperationException("The model must be loaded before the feed-forward norm layer can be created.");
            }
        ...
        public void Forward(BitNetSession session)
        {
            ...
            ForwardCore(session.FeedForwardInput, session.FeedForwardNorm);
        }
        ...
        private void ExecuteForward(ReadOnlyMemory<float> input, ReadOnlyMemory<float> normWeights, Memory<float> output)
        {
            ...
            switch (InferenceConfig.Backend)
            {
                ...
                case InferenceBackend.Tensor:
                    ...
                    return;
                default:
                    throw new NotSupportedException($"Feed-forward norm backend '{InferenceConfig.Backend}' is not implemented yet.");
            }
        }
        ...
        private float[] ReadNormWeights()
        {
            ...
            return normTensor.TensorType switch
            {
                ...
                _ => throw new NotSupportedException($"Feed-forward norm tensor type '{normTensor.TensorType}' is not supported."),
            };
        }
        ...
        private static void FillFloatValues(ReadOnlySpan<byte> source, GGUFTensorType tensorType, Span<float> destination)
        {
            switch (tensorType)
            {
                ...
                case GGUFTensorType.GGML_TYPE_F16:
                    ...
                    return;
                default:
                    throw new NotSupportedException($"Feed-forward norm tensor type '{tensorType}' is not supported.");
            }
        }
